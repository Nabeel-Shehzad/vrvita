  ...
  void _onItemTapped(int index) {
    ...
    switch (index) {
      ...
      // ✅ أيقونة “Appointments” تفتح صفحة المواعيد
        Navigator.push(context, MaterialPageRoute(builder: (_) => const VrUpcomingAppointmentsPage()));
        break;
      case 2:
  ...
  static void _open(BuildContext context, String title) {
    switch (title) {
      case "Quiz Scores (Team)":
        Navigator.push(context, MaterialPageRoute(builder: (_) => const QuizScorePage(role: UserRole.supervisor)));
        return;
      case "Team Reports":
        Navigator.push(context, MaterialPageRoute(builder: (_) => const TeamReportPage()));
        return;
      case "Approve Sessions":
        Navigator.push(context, MaterialPageRoute(builder: (_) => const ApproveSessionPage()));
        return;
      case "Contact Us":
        Navigator.push(context, MaterialPageRoute(builder: (_) => const ContactUsPage(role: UserRole.supervisor)));
        return;
      case "Appointments":
...
class _MainDrawer extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    ListTile item(IconData icon, String title, String pageTitle, {VoidCallback? onTap}) => ListTile(
      leading: Icon(icon, color: kBrand),
      title: Text(title, style: const TextStyle(fontWeight: FontWeight.w600, color: kBrand)),
      onTap: onTap ??
              () {
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (_) => _PlaceholderPage(title: pageTitle)));
          },
    );
          ...
          children: [
            ListTile(
              leading: const CircleAvatar(
                  backgroundColor: Color(0xFF9FBEEC), child: Icon(Icons.person, color: kBrand)),
              trailing: IconButton(
            ...
            item(Icons.menu_book_rounded, "Tutorials", "Tutorials", onTap: () {
              Navigator.pop(context);
              Navigator.push(context, MaterialPageRoute(builder: (_) => const TutorialPage()));
            }),
            item(Icons.help_outline_rounded, "FAQs", "FAQs", onTap: () {
              Navigator.pop(context);
              Navigator.push(context, MaterialPageRoute(builder: (_) => const FaqsPage()));
            }),

            // ✅ Contact Us الحقيقي بدور Supervisor
            item(Icons.support_agent_rounded, "Contact Us", "Contact Us", onTap: () {
              Navigator.pop(context);
              Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const ContactUsPage(role: UserRole.supervisor)),
              );
            }),
            ...
            // إضافات المشرف
            item(Icons.leaderboard_rounded, "Quiz Scores (Team)", "Quiz Scores (Team)", onTap: () {
              Navigator.pop(context);
              Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const QuizScorePage(role: UserRole.supervisor)),
              );
            }),
            item(Icons.assessment_rounded, "Team Reports", "Team Reports", onTap: () {
              Navigator.pop(context);
              Navigator.push(context, MaterialPageRoute(builder: (_) => const TeamReportPage()));
            }),
            item(Icons.verified_rounded, "Approve Sessions", "Approve Sessions", onTap: () {
              Navigator.pop(context);
              Navigator.push(context, MaterialPageRoute(builder: (_) => const ApproveSessionPage()));
            }),
            const Divider(),
